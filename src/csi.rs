...
pub fn parse_csi_protobuf(expected_protobuf: &[u8]) -> Result<CSIMessage, protobuf::Error>  {
...
fn get_csi_matrix(msg: &CSIMessage) -> Result<Array<f32, Ix2>, RecvMessageError> {
    let csi_data = msg.csi_data.clone().unwrap_or_else(RecvMessageError::CSIReadingGenerateError(msg.clone()));
...
pub fn get_correlation_coefficient(frame: Array<f32, Ix2>, frame2: &Array<f32, Ix2>) -> Result<f32, RecvMessageError> {
...
pub fn get_reading(msg: &CSIMessage) -> Result<CSIMessageReading, RecvMessageError> {
    ...
    let mac = format!("0x{:X}", msg.src_mac.clone().unwrap()[5]);
